...
- `max-date YYYY-MM-DD [HH:MM]` whitelist rule: allows only versions published on or before the given UTC date/time. Time defaults to `12:00` when omitted. Can be combined with exact versions and `min-age` in an array entry. Applies to both manifest filtering and tarball requests.
